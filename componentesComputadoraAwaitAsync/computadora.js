...
const makeRequest = async event => {
  try {
    const requestType = await resolveMakeRequest(event)
    switch (requestType) {
      case 'RAM':
        getRAMData()
        break
...
const getRAMData = async () => {
  try {
    const response = await ajax('GET', 'https://demo6292426.mockable.io/ramMemory')
    const data = Array.from(response.data).reduce((main, elemento) => {
      main.push(`${elemento.ramMemory} ${elemento.architecture} ${elemento.speed}`)
      return main
    }, [])
    showModal(arrayToHTML('RAM', data), 'Agregar RAM')
  } catch (error) {
    console.error(error)
  }
}
...
const getHDDData = async () => {
  try {
    const response = await ajax('GET', 'http://demo6292426.mockable.io/hddList')
    const data = Array.from(response.data).reduce((main, elemento) => {
      main.push(`${elemento.size} ${elemento.provider} ${elemento.speed}`)
      return main
    }, [])
    showModal(arrayToHTML('HDD', data), 'Agregar HDD')
  } catch (error) {
    console.error(error)
  }
}
...
const getProcessorData = async () => {
  try {
    const response = await ajax('GET', 'http://demo6292426.mockable.io/processors')
    const data = Array.from(response.data).reduce((main, elemento) => {
      main.push(`${elemento.architecture} ${elemento.provider} ${elemento.cores} núcleos`)
      return main
    }, [])
    showModal(arrayToHTML('PROCESSOR', data), 'Agregar PROCESSOR')
  } catch (error) {
    console.error(error)
  }
}
...
const resolveCloseModal = async () => {
  try {
    const modal = await closeModal()
    modal.remove()
  } catch (error) {
    alert(error)
  }
}
const resolveAcceptModal = async () => {
  try {
    const value = await acceptModal()
...
const setLabelValue = async type => {
  try {
    const text = await resolveValueLabel(type)
